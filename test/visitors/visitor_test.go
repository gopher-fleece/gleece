...
var _ = Describe("Visitor Tests", func() {
	...
	var _ = Context("Route Visitor", func() {
		...
		It("Ignores receivers with no annotations", func() {
			meta := utils.GetMetadataByRelativeConfigOrFail("./configs/receiver.with.no.comments.json")
			Expect((meta.Flat[0].Routes)).ToNot(ContainElement(Satisfy(func(route definitions.RouteMetadata) bool {
				return route.OperationId == "NotAnApiMethod"
			})))
		})

		It("Returns a correct error when a receiver annotation has invalid JSON", func() {
			_, err := utils.GetMetadataByRelativeConfig("./configs/receiver.with.invalid.json.json")
		...
		It("Ignores receivers with no @Method annotation", func() {
			meta := utils.GetMetadataByRelativeConfigOrFail("./configs/receiver.with.no.method.annotation.json")
			Expect((meta.Flat[0].Routes)).ToNot(ContainElement(Satisfy(func(route definitions.RouteMetadata) bool {
		...
		It("Ignores receivers with no @Route annotation", func() {
			meta := utils.GetMetadataByRelativeConfigOrFail("./configs/receiver.with.no.route.annotation.json")
			Expect((meta.Flat[0].Routes)).ToNot(ContainElement(Satisfy(func(route definitions.RouteMetadata) bool {
		...
		Context("Return Type Checks", func() {
			It("Returns a correct error when a receiver returns void", func() {
				_, err := utils.GetMetadataByRelativeConfig("./configs/receiver.with.void.return.json")
				errStr := err.Error()
			...
			It("Returns a correct error when a receiver has one return type that is not a error", func() {
				_, err := utils.GetMetadataByRelativeConfig("./configs/receiver.with.non.error.return.json")
				errStr := err.Error()
