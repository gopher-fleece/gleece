...
func RegisterRoutes(engine *mux.Router) {
	...
	engine.HandleFunc(toMuxUrl("/e2e/custom-context-1"), func(w http.ResponseWriter, ctx *http.Request) {
		// route start routes extension placeholder
		authErr := authorize(
			ctx,
			[]SecurityCheckList{
		...
		if authErr != nil {
			handleAuthorizationError(w, authErr, "CustomContext1")
			return
		}
		controller := E2EControllerImport.E2EController{}
		controller.InitController(ctx)
		...
		w.Header().Set("x-inject", "true")
		w.Header().Set("x-extended", "CustomContext1")
	...
	engine.HandleFunc(toMuxUrl("/e2e/custom-context-2"), func(w http.ResponseWriter, ctx *http.Request) {
		// route start routes extension placeholder
		authErr := authorize(
			ctx,
			[]SecurityCheckList{
		...
		if authErr != nil {
			handleAuthorizationError(w, authErr, "CustomContext2")
			return
		}
		controller := E2EControllerImport.E2EController{}
		controller.InitController(ctx)
		...
		w.Header().Set("x-inject", "true")
		w.Header().Set("x-extended", "CustomContext2")
