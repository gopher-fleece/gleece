...
func RegisterRoutes(engine *echo.Echo) {
	...
	engine.POST(toEchoUrl("/e2e/external-packages-unique-in-struct"), func(ctx echo.Context) error {
		ctx.Response().Header().Set("x-RouteStartRoutesExtension", "ExternalPackagesUniqueInStruct")
		authErr := authorize(
			ctx,
			[]SecurityCheckList{
				{
		...
		if conversionErr != nil {
			...
			ctx.Response().Header().Set("x-JsonBodyValidationErrorResponseExtension", "ExternalPackagesUniqueInStruct")
			return ctx.JSON(http.StatusUnprocessableEntity, validationError)
		}
		// Middlewares beforeOperationMiddlewares section
		for _, middleware := range beforeOperationMiddlewares {
		...
		// End middlewares beforeOperationMiddlewares section
		ctx.Response().Header().Set("x-BeforeOperationRoutesExtension", "ExternalPackagesUniqueInStruct")
		value, opError := controller.ExternalPackagesUniqueInStruct(*dataRawPtr)
		for key, value := range controller.GetHeaders() {
			ctx.Response().Header().Set(key, value)
		}
		ctx.Response().Header().Set("x-inject", "true")
		ctx.Response().Header().Set("x-ResponseHeadersExtension", "ExternalPackagesUniqueInStruct")
		statusCode := getStatusCode(&controller, true, opError)
		if opError != nil {
			// Middlewares onErrorMiddlewares section
			for _, middleware := range onErrorMiddlewares {
			...
			ctx.Response().Header().Set("x-JsonErrorResponseExtension", "ExternalPackagesUniqueInStruct")
			return ctx.JSON(statusCode, stdError)
		}
		ctx.Response().Header().Set("x-JsonResponseExtension", "ExternalPackagesUniqueInStruct")
		// Middlewares afterOperationSuccessMiddlewares section
		for _, middleware := range afterOperationSuccessMiddlewares {
			if continueOperation := middleware(ctx); !continueOperation {
				return nil
		...
		// End middlewares afterOperationSuccessMiddlewares section
		ctx.Response().Header().Set("x-AfterOperationRoutesExtension", "ExternalPackagesUniqueInStruct")
		ctx.JSON(statusCode, value)
		ctx.Response().Header().Set("x-RouteEndRoutesExtension", "ExternalPackagesUniqueInStruct")
		return nil
	})
