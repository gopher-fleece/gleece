...
var _ = BeforeSuite(func() {
	...
	gleeceRoutes.RegisterRoutes(e)
})

var _ = Describe("Echo E2E Spec", func() {
	...
	It("Should return status code 200 for get with all params in use", func() {
		...
		e.ServeHTTP(w, req)
		Expect(w.Code).To(Equal(200))
		Expect(w.Body.String()).To(ContainSubstring("\"pathParamqueryParamheaderParam\""))
	})
	...
	It("Should return status code 200 for get with all params empty ptr", func() {
		...
		e.ServeHTTP(w, req)
		Expect(w.Code).To(Equal(200))
		Expect(w.Body.String()).To(ContainSubstring("\"pathParam\""))
	})
